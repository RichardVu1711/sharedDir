...
void duplicate_data(Mat* in, int sel,
					fixed_type prtcls_0[NUM_VAR],
					fixed_type prtcls_1[NUM_VAR])
{
#pragma HLS inline off
	...
	{
#pragma HLS pipeline
		prtcls_0[i0] = in->entries[i0*NUM_PARTICLES+sel];
		prtcls_1[i0] = in->entries[i0*NUM_PARTICLES+sel];
	}
}
...
#pragma HLS INTERFACE mode=m_axi bundle=gmem1 port=R_mat offset=slave
#pragma HLS INTERFACE mode=m_axi bundle=gmem1 port=Pxx_  offset=slave
#pragma HLS INTERFACE mode=m_axi bundle=gmem1 port=msmtinfo offset=slave
...
#pragma HLS DATAFLOW
		...
		duplicate_data(&prtcls_local,j1,prtcls_0,prtcls_1);

//		Calculate the estimated measurement based on the given particles
		ObsJacobian(prtcls_0,idx1,&msmtinfo1,obsJacobian);
		GISobs_model(prtcls_1,idx2,&msmtinfo2,GISobs);
