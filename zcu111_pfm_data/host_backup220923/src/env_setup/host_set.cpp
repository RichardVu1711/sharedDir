			    ...
			    cl::Program& program_lc)
{
	    ...
	    devices_lc = xcl::get_xil_devices();
	    // read_binary_file() is a utility API which will load the binaryFile
	    // and will return the pointer to file buffer.
	    auto fileBuf = xcl::read_binary_file(binaryFile);
	    cl::Program::Binaries bins{{fileBuf.data(), fileBuf.size()}};
	    bool valid_device = false;
	    for (unsigned int i = 0; i < devices_lc.size(); i++) {
	        device_lc = devices[i];
	        // Creating Context and Command Queue for selected Device
	        OCL_CHECK(err, context_lc = cl::Context(device_lc, nullptr, nullptr, nullptr, &err));
	        for(int j=0; j < Q_LEN;j++){
	        	OCL_CHECK(err, q_lc[j] = cl::CommandQueue(context_lc, device_lc, CL_QUEUE_PROFILING_ENABLE, &err));
	        }
	        std::cout << "Trying to program device[" << i << "]: " << device_lc.getInfo<CL_DEVICE_NAME>() << std::endl;
	        OCL_CHECK(err, program_lc = cl::Program(context_lc, {device_lc}, bins, nullptr, &err));
	        if (err != CL_SUCCESS) {
	            std::cout << "Failed to program device[" << i << "] with xclbin file!\n";
	        } else {
	            std::cout << "Device[" << i << "]: program successful!\n";
//	            OCL_CHECK(err, cl::Kernel kCal = cl::Kernel(program,"CalPzxZdiff", &err));
	            std::cout << "CalPzxZdiff[" << i << "]: program successful!\n";
			...
			cl::Kernel& kSigma,cl::Kernel& kCreate, int qIdx)
{
	...
	write_csv("/home/petalinux/result/sigMat.csv",convert_double(sigMat,1,13*1024,0),13,1024);
	...
	write_csv("/home/petalinux/result/prtcls.csv",convert_double(prtcls,1,13*1024,0),13,1024);
	cout << "\nFinished Sampling\n";
...
// mvnpdf
	...
	write_csv("/home/petalinux/result/zDiff.csv",convert_double(zDiff,1,6*1024,-1),1024,6);
	write_csv("/home/petalinux/result/pzx.csv",convert_double(pzx,1,36*1024,-1),6*1024,6);
	write_csv("/home/petalinux/result/wt.csv",convert_double(wt,1,1*1024,-1),1,1024);
			...
			cl::Kernel& kPFU, int qIdx)
{
	...
	write_csv("/home/petalinux/result/state_sol.csv",convert_double(state,1,13,-1),1,13);
	write_csv("/home/petalinux/result/pxx_sol.csv",convert_double(pxx,1,13*13,-1),13,13);
